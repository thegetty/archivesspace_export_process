...
<!--        <xsl:choose>
        ...
        <xsl:apply-templates select="ead:titleproper[not(@type)]"/>
    </xsl:template>
...
<!--                <xsl:choose>
    ...
    <xsl:template name="icon">
        <fo:block text-align="left" margin-left="3in" margin-top="-.5in">
            <fo:external-graphic src='url("http://archives2.getty.edu:8082/xtf/icons/default/logo_sm.gif")' 
                        ...
                        <fo:bookmark-title>
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    ...
                                    <xsl:value-of select="ead:did/ead:unittitle | ead:did/ead:unitdate"/>
                                </xsl:otherwise>
                            </xsl:choose>    
                        </fo:bookmark-title>
                    </fo:bookmark>
                        ...
                        <fo:block text-align-last="justify" margin-left="8pt"> 
                            <fo:basic-link internal-destination="{local:buildID(.)}">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:apply-templates select="child::*/ead:unittitle  | child::*/ead:unitdate" />
                                    </xsl:otherwise>
                                </xsl:choose>    
                            </fo:basic-link>                    
                            <xsl:text>&#160;&#160;</xsl:text>                    
        ...
        <fo:block xsl:use-attribute-sets="h2ID">Summary Information</fo:block>
            ...
            <fo:table table-layout="fixed" width="100%">
                ...
                <fo:table-body>
                    ...
                    <xsl:apply-templates select="child::ead:origination/*[@role = 'cre'] | child::ead:origination/*[@role = 'com'] | child::ead:origination/*[@role = 'col']" mode="overview"/>
                        <xsl:apply-templates select="ead:unittitle" mode="overview"/>    
                    <xsl:apply-templates select="ead:unitid[not(@identifier)]" mode="overview"/>
    ...
    <xsl:template match="ead:repository | child::ead:origination/*[@role = 'cre'] 
        | child::ead:origination/*[@role = 'com'] | child::ead:origination/*[@role = 'col'] 
        | ead:unitdate[not(self::ead:unitdate[@type='bulk'])] | ead:unittitle | ead:unitid[not(@identifier)]  
        | ead:physdesc | ead:physloc | ead:dao | ead:daogrp | ead:materialspec | ead:container 
        | ead:abstract | ead:note" mode="overview">
        <fo:table-row>
            <fo:table-cell padding-bottom="10pt" text-align="right" font-weight="bold">
                <fo:block>
<!--                <xsl:choose>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:choose>
                                ...
                                <xsl:when test="self::ead:unitid[not(@identifier)]">ID</xsl:when>
                                <xsl:when test="self::ead:unitdate">Dates</xsl:when>
                                <xsl:when test="self::ead:persname[@role = 'cre'] | self::ead:corpname[@role = 'cre']">Creator</xsl:when>
                                <xsl:when test="self::ead:persname[@role = 'com'] | self::ead:corpname[@role = 'com']">Collector</xsl:when>
                                <xsl:when test="self::ead:persname[@role = 'col'] | self::ead:corpname[@role = 'col']">Collector</xsl:when>
                                <xsl:when test="self::ead:physdesc">Extent</xsl:when>
                                <xsl:when test="self::ead:abstract">Abstract</xsl:when>
                                <xsl:when test="self::ead:physloc">Request Materials</xsl:when>
    ...
    <xsl:template match="ead:langmaterial" mode="overview">
        <fo:table-row>
            <fo:table-cell padding-bottom="10pt" text-align="right">
                <fo:block font-size="12pt" font-weight="bold" color="#111">
                    <xsl:if test="position() = last()">
                        <xsl:text>Language:</xsl:text></xsl:if>
                </fo:block>
            </fo:table-cell>
...
<!--    <xsl:template match="ead:publicationstmt">
    ...
    <!-- Templates for revisiondescription  -->
    <xsl:template match="ead:revisiondesc">
        <fo:block xsl:use-attribute-sets="section">
            <fo:block xsl:use-attribute-sets="h3ID"><xsl:value-of select="local:tagName(.)"/></fo:block>
            <xsl:if test="ead:change/ead:date"><fo:block xsl:use-attribute-sets="h4"><xsl:value-of select="ead:change/ead:date"/></fo:block></xsl:if>
            <xsl:if test="ead:change/ead:item"><fo:block xsl:use-attribute-sets="smp"><xsl:value-of select="ead:change/ead:item"/></fo:block></xsl:if>
                    </fo:block>
    </xsl:template>
...
<!--    <xsl:template match="ead:controlaccess">
    ...
    <xsl:template match="ead:controlaccess">
        ...
        <xsl:if test="../ead:did/ead:origination/ead:persname[@role = 'ctb'] | ../ead:did/ead:origination/ead:corpname[@role = 'ctb']">
            <fo:block xsl:use-attribute-sets="h4"> Contributor(s) </fo:block>
            <fo:list-block xsl:use-attribute-sets="smp">
                <xsl:for-each select="../ead:did/ead:origination/ead:persname[@role = 'ctb'] | ../ead:did/ead:origination/ead:corpname[@role = 'ctb']">
                    <xsl:sort/>
                    <fo:list-item>
                        <fo:list-item-label end-indent="label-end()">
                            <fo:block>&#x2022;</fo:block>
            ...
            <xsl:if test="../ead:controlaccess/ead:persname[not(@role)]">
                <fo:block xsl:use-attribute-sets="h4"> Subjects - People</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="../ead:controlaccess/ead:persname[not(@role)]">
                        <xsl:sort/>
                        <fo:list-item>
                            <fo:list-item-label end-indent="label-end()">
                                <fo:block>&#x2022;</fo:block>
            ...
            <xsl:if test="../ead:controlaccess/ead:corpname[not(@role)]">
            <fo:block xsl:use-attribute-sets="h4"> Subjects - Corporate Bodies </fo:block>
            <fo:list-block xsl:use-attribute-sets="smp">
                <xsl:for-each select="../ead:controlaccess/ead:corpname[not(@role)]">
                    <xsl:sort/>
                    <fo:list-item>
                        <fo:list-item-label end-indent="label-end()">
                            <fo:block>&#x2022;</fo:block>
            ...
            <xsl:if test="../ead:controlaccess/ead:famname[not(@role)]">
            <fo:block xsl:use-attribute-sets="h4"> Subjects - Family Names </fo:block>
            <fo:list-block xsl:use-attribute-sets="smp">
                <xsl:for-each select="../ead:controlaccess/ead:famname[not(@role)]">
                    <xsl:sort/>
                    <fo:list-item>
                        <fo:list-item-label end-indent="label-end()">
                            <fo:block>&#x2022;</fo:block>
            ...
            <xsl:if test="ead:subject">
                <fo:block xsl:use-attribute-sets="h4"> Subjects - Topics</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
            ...
            <xsl:if test="ead:geogname">
                <fo:block xsl:use-attribute-sets="h4"> Subjects - Places</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
        ...
        <xsl:if test="ead:genreform">
            <fo:block xsl:use-attribute-sets="h4"> Genres and Forms of Materials</fo:block>
            <fo:list-block xsl:use-attribute-sets="smp">
            ...
            <xsl:if test="ead:function">
                <fo:block xsl:use-attribute-sets="h4"> Function(s)</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
        ...
        <xsl:if test="ead:occupation">
            <fo:block xsl:use-attribute-sets="h4"> Occupation(s)</fo:block>
            <fo:list-block xsl:use-attribute-sets="smp">
    ...
    <xsl:template match="ead:table">  
        <xsl:for-each select="ead:table/ead:head">
...
<!--               <xsl:for-each select="ead:colspec">
                 ...
                 <fo:table-header> 
                     <xsl:for-each select="ead:thead/ead:row">
            ...
            <xsl:for-each select="ead:tbody/ead:row">        
            <fo:table-body>
                        <fo:table-row>
                            <xsl:for-each select="ead:entry">
                                <fo:table-cell padding="4pt" xsl:use-attribute-sets="tdBorder">
                                    <fo:block>
                                        <xsl:value-of select="."/>
                                    </fo:block>
                         </fo:table-cell>   </xsl:for-each>                     
...
<!--    <xsl:template match="ead:row" mode="thead">
   ...
   <!-- Linking elmenets -->
    <xsl:template match="ead:ref">
        <fo:basic-link internal-destination="{@xlink:target}" xsl:use-attribute-sets="ref">
            <xsl:choose>
                <xsl:when test="text()">
                    <xsl:value-of select="."/>
                </xsl:when>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="@xlink:target"/>
                </xsl:otherwise>
            </xsl:choose>
        </fo:basic-link>
    </xsl:template>
    ...
    <xsl:template match="ead:title[not(@render)]"><fo:inline font-style="italic"><xsl:apply-templates/></fo:inline></xsl:template>
        ...
        <xsl:variable name="clevelMargin">
            ...
            <xsl:choose>
                ...
                <xsl:when test="@level='subcollection' or @level='subgrp' or @level='series' 
                    or @level='subseries' or @level='collection'or @level='fonds' or @level='recordgrp' or @level='subfonds' or @level='class'">
                    ...
                    <xsl:if test="ead:did/ead:container">
                        <xsl:choose>
                            <xsl:when test="ead:did/ead:container/@label">
                                <xsl:for-each-group select="ead:did/ead:container" group-starting-with="ead:did/ead:container[@label]">
                                    <fo:table-row background-color="#f0f0f0" border-bottom="1px dotted #ccc" border-top="3pt solid #ccc" keep-with-next.within-page="always">
                    ...
                    <!-- Adds column headings if series/subseries is followed by an item.  Working here NE 11/28/2016. -->
                    <xsl:if test="child::*[@level][1][@level='item' or @level='file' or @level='otherlevel']">
                        <xsl:call-template name="tableHeaders"/>
                    </xsl:if>
                </xsl:when>
...
<!--                        <fo:table-cell>
    ...
    <xsl:template match="ead:container">
        <fo:table-cell>
            <fo:block margin="4pt 0"><xsl:value-of select="concat(upper-case(substring(@type,1,1)),substring(@type,2))"/><xsl:text> </xsl:text><xsl:value-of select="."/></fo:block>
        </fo:table-cell>
    </xsl:template>
    
    <!-- Series titles -->
    <xsl:template match="ead:did" mode="dscSeriesTitle">
        <fo:block font-weight="bold" font-size="16" margin-bottom="0" margin-top="4" id="{local:buildID(parent::*)}">
            <!-- Uncomment the following to add 'Series' to series titles.  In line above, changed font size from 20 to 16  -->
           
            <xsl:if test="ead:unitid[not(@identifier)]">
                <xsl:choose>
                    <xsl:when test="../@level='series'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>&#160; </xsl:when>
                    <xsl:when test="../@level='subseries'"> <xsl:value-of select="ead:unitid[not(@identifier)]"/> &#160;</xsl:when>
                    <xsl:when test="../@level='subsubseries'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>&#160; </xsl:when>
                    <xsl:when test="../@level='collection'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>&#160; </xsl:when>
                    <xsl:when test="../@level='subcollection'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>:&#160; </xsl:when>
                    <xsl:when test="../@level='fonds'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>:&#160; </xsl:when>
                    <xsl:when test="../@level='subfonds'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>:&#160; </xsl:when>
                    <xsl:when test="../@level='recordgrp'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>&#160; </xsl:when>
                    <xsl:when test="../@level='subgrp'"><xsl:value-of select="ead:unitid[not(@identifier)]"/>&#160; </xsl:when>
                    <xsl:otherwise><xsl:value-of select="ead:unitid[not(@identifier)]"/>: &#160;</xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            
            <xsl:apply-templates select="ead:unittitle" />  <xsl:text> </xsl:text>
            <xsl:if test="(string-length(ead:unittitle[not(self::ead:unitdate[@type='bulk'])]) &gt; 1) and (string-length(ead:unitdate[not(self::ead:unitdate[@type='bulk'])]) &gt; 1)">  </xsl:if>
            <xsl:apply-templates select="ead:unitdate[not(self::ead:unitdate[@type='bulk'])]" mode="did"/>  <xsl:text> &#160;&#160;&#160;  </xsl:text>
            <xsl:apply-templates select="ead:physdesc" mode="did"/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="ead:did" mode="dsc">
        <fo:block margin-bottom="0">
            <xsl:if test="ead:unitid[not(@identifier)]">
                <xsl:apply-templates select="ead:unitid[not(@identifier)]"/> <xsl:text>: </xsl:text> </xsl:if>
            <xsl:apply-templates select="ead:unittitle" /> <xsl:text> </xsl:text>
            <xsl:apply-templates select="ead:unitdate" mode="did"/><xsl:text>     </xsl:text>
             <xsl:apply-templates select="ead:physdesc" mode="did"/>
        </fo:block> 
        <fo:block margin-bottom="4pt" margin-top="0">
            <xsl:apply-templates select="ead:repository" mode="dsc"/>            
...
<!--    <xsl:template match="ead:unitdate[@type = 'bulk']" mode="did">
    ...
    <xsl:template match="ead:repository | ead:origination | ead:unitdate | ead:unitid[not(@identifier)]  
       | ead:physloc | ead:langmaterial | ead:materialspec | ead:container 
        | ead:abstract | ead:note" mode="dsc">
        <xsl:choose>
            <xsl:when test="@audience"></xsl:when>
            <xsl:otherwise>
                 <xsl:if test="child::*">
    ...
    <xsl:template match="ead:scopecontent | ead:bioghist | ead:arrangement | ead:relatedmaterial | ead:separatedmaterial | ead:accessrestrict | ead:userestrict | ead:phystech |
        ead:custodhist | ead:accruals | ead:altformavail | ead:acqinfo |  
        ead:processinfo | ead:appraisal | ead:odd | ead:originalsloc" mode="dsc">
        <xsl:choose>
            <xsl:when test="@audience"></xsl:when>
            <xsl:otherwise>
                 <xsl:if test="child::*">
                <fo:block xsl:use-attribute-sets="smpDsc">
                <fo:inline font-style="italic"><xsl:value-of select="local:tagName(.)"/></fo:inline><xsl:text>: </xsl:text>
...
<!--    <xsl:template match="ead:controlaccess" mode="dsc">         
    ...
    <xsl:template match="ead:controlaccess" mode="dsc">
        <fo:block xsl:use-attribute-sets="section"> 
            <xsl:if test="../ead:did/ead:origination/ead:persname[@role = 'ctb'] | ../ead:did/ead:origination/ead:corpname[@role = 'ctb']">
                <fo:block xsl:use-attribute-sets="smpDsc"> Contributor(s) </fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="../ead:did/ead:origination/ead:persname[@role = 'ctb'] | ../ead:did/ead:origination/ead:corpname[@role = 'ctb']">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="../ead:controlaccess/ead:persname[not(@role)]">
                <fo:block xsl:use-attribute-sets="smpDsc"> Subjects - People</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="../ead:controlaccess/ead:persname[not(@role)]">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="../ead:controlaccess/ead:corpname[not(@role)]">
                <fo:block xsl:use-attribute-sets="smpDsc"> Subjects - Corporate Bodies </fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="../ead:controlaccess/ead:corpname[not(@role)]">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="../ead:controlaccess/ead:famname[not(@role)]">
                <fo:block xsl:use-attribute-sets="smpDsc"> Subjects - Family Names </fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="../ead:controlaccess/ead:famname[not(@role)]">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="ead:subject">
                <fo:block xsl:use-attribute-sets="smpDsc"> Subjects - Topics</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="ead:subject">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="ead:geogname">
                <fo:block xsl:use-attribute-sets="smpDsc"> Subjects - Places</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="ead:geogname">
                        <xsl:sort/>
                        <fo:list-item>
            ...
            <xsl:if test="ead:genreform">
                <fo:block xsl:use-attribute-sets="smpDsc"> Genres and Forms of Materials</fo:block>
                <fo:list-block xsl:use-attribute-sets="smp">
                    <xsl:for-each select="ead:genreform">
                        <xsl:sort/>
                        <fo:list-item>
                ...
                <xsl:if test="ead:function">
                    <fo:block xsl:use-attribute-sets="smpDsc"> Function(s)</fo:block>
                    <fo:list-block xsl:use-attribute-sets="smp">
                        <xsl:for-each select="ead:function">
                            <xsl:sort/>
                            <fo:list-item>
                ...
                <xsl:if test="ead:occupation">
                    <fo:block xsl:use-attribute-sets="smpDsc"> Occupation(s)</fo:block>
                    <fo:list-block xsl:use-attribute-sets="smp">
                        <xsl:for-each select="ead:occupation">
                            <xsl:sort/>
                            <fo:list-item>
